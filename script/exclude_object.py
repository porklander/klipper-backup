...
class ExcludeObject:
    ...
    def _normal_move(self, newpos, speed):
        ...
        if self.initial_extrusion_moves > 0 and \
            self.gcode_move.absolute_extrude and \
    ...
    def _test_in_excluded_region(self):
        # Inside cancelled object, and relative extrusion or absolute
        # with enough tracked extrusions
        return self.current_object in self.excluded_objects \
            and not (self.gcode_move.absolute_extrude \
                     and self.initial_extrusion_moves)
